...
void HyUCC::FitInternal(model::IDatasetStream& data_stream) {
...
unsigned long long HyUCC::ExecuteInternal() {
    ...
    algos::Sampler sampler(plis_shared, pli_records_shared, threads_num_);
    ...
    while (true) {
        LOG(DEBUG) << "Sampling...";
        NonUCCList non_uccs = sampler.GetNonUCCCandidates(comparison_suggestions);
