...
// Checkout comment for Validator::ValidateAndExtendCandidates, it's appropriate for this function
// too
...
Validator::UCCValidations Validator::ValidateAndExtendParallel(
    ...
    // split current_level on 4 spans
    for (auto const& vertex_and_ucc : current_level) {
        if (!vertex_and_ucc.first->IsUCC()) {
            continue;
        }
...
hy::IdPairs Validator::ValidateAndExtendCandidates() {
    ...
    while (!current_level.empty()) {
        ...
        if (num_invalid_uccs >
                    (long double)hyfd::HyFDConfig::kEfficiencyThreshold * num_valid_uccs &&
